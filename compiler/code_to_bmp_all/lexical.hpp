...
struct Lexical {
    ...
    void pushHex(std::string color, int& numLine) {
        while (hex.size() < numLine) {
            hex.push_back({});
        }

        hex.back().push_back(color);
    }
            ...
            if (s == 0) {
                ...
                if (keyword.count(str) || col == lenLine - 1 || !std::isalnum(nxt)) {
                    ...
                    if (keyword.count(str)) {
                        ...
                        pushTokenData(str, s, numLine, col, token::DataType::UNDEFINEDDT);
                    }
